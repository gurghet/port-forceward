...
object PortForceward extends ZIOCliDefault {
  ...
  val resourceTypeOpt =
    Options.text("resource-type").alias("t", "type").withDefault("pod") ?? "The resource type to forward"

  val resourceNameOpt =
    Options.text("resource-name").alias("r", "res", "resource") ?? "The resource name to forward"

  val portOpt =
    Options.text("ports").alias("p", "port").mapOrFail{
      case s if s.matches("\\d+:\\d+") => Right(s)
      case _ => Left(ValidationError(InvalidArgument, HelpDoc.p("Ports must be in the format 'local:remote'")))
  ...
  val forceward =
    Command("forceward", resourceTypeOpt ++ resourceNameOpt ++ portOpt ++ kubeConfigOpt, Args.none)

  val helpDoc = HelpDoc.p("A forceful port forwarder for kubectl")

  def portForwardResource(rscType: String, rscName: String, ports: String, kubeconfig: Option[String]) =
    val args = List(
      "port-forward",
      rscType + "/" + rscName,
      ports) ++ kubeconfig.map(k => List("--kubeconfig", k)).getOrElse(List.empty)
  ...
  def verifyResourceTypeExists(resType: String, kubeconfigOpt: Option[String]) =
    getResourceTypesCommand(kubeconfigOpt)
      .run
      .flatMap { handle =>
        handle.stdout
          .linesStream
          .map(_.split("\\s+"))
          .runCollect
          .map(chunk => chunk.flatMap(arr => Chunk.fromArray(arr)))
          .map(_.flatMap(resTypeMaybePlural => 
            if resTypeMaybePlural.endsWith("s") then Chunk(resTypeMaybePlural, resTypeMaybePlural.dropRight(1)) 
            else Chunk(resTypeMaybePlural))
          ).map(_.contains(resType))
      }

  def getResourceListCommand(resourceType: String, kubeconfigOpt: Option[String]) =
    val args = List(
      "get",
      resourceType,
      "--no-headers",
  ...
  def findSimilarlyNamedResource(res: String, list: Chunk[String]): Option[String] =
    list
      .map { r =>
        val distance = org.apache.commons.text.similarity.LevenshteinDistance.getDefaultInstance().apply(r, res)
        (r, distance)
      }
      .sortBy(_._2)
      .headOption
  ...
  def findResourceOrSuggestSimilarlyNamed(resourceType: String, resourceName: String, kubeconfigOpt: Option[String]) =
    ...
    for {
      ...
      _ <- if resources.contains(resourceName) then ZIO.unit
      else
        val suggestionString = findSimilarlyNamedResource(resourceName, resources)
  ...
  def cliApp = CliApp.make(
    ...
    case (rscTypeIn, rscNameIn, ports, kubeconfig) => for {
      _ <- printLine(s"User input: '$rscTypeIn' '$rscNameIn' '$ports' '$kubeconfig'")
  ...
  def forcewardWithTypeAndName(rscType: String, rscName: String, ports: String, kubeconfig: Option[String]) = for {
      ...
      trialLoop = for {
        ...
        proc <- tryToPortForwardAndGiveUpAfter5Seconds(rscType, rscName, ports, kubeconfig)
          .tapError { _ =>
            for {
